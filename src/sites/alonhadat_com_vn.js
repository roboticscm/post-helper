...
export const alonhadat_com_vn = async (browser, postId, url, username, password, post, config, autoClose) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        // click login link
        const [loginButton] = await page.$x('//*[@id="ctl00_pc_header"]/div[2]/div/a[3]')
        ...
        await page.waitForSelector('a[href="/dang-tin-nha-dat.html"]');
        await page.click('a[href="/dang-tin-nha-dat.html"]');
        ...
        if (post.description) {
            try {    
                await page.evaluate((value) => document.querySelector('#noidung').value = value, post.description)
            } catch (err) {
                console.error(err);
            }
        }
        ...
        if (post.street) {
            try {
                ...
                await page.waitForTimeout(500);
                await selectOptionByText(page, 'street_acp_text', post.street);
            } catch (err) {
                console.error(err);
            }
        }
...
module.exports = alonhadat_com_vn;